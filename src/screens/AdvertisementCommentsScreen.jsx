...
} from "../../services/advertisementCommentsApi";
...
export default function AdvertisementCommentsScreen() {
  ...
  // API hooks
  const { data: commentsData, isLoading, error, refetch } = useGetAdvertisementCommentsQuery(
    { advertisementId, page: 1, limit: 50 },
    { 
      skip: !advertisementId || !user, // Skip if no advertisementId or user not authenticated
      // Add error handling
      onError: (error) => {
  ...
  const [createComment, { isLoading: isCreatingComment }] = useCreateAdvertisementCommentMutation();
  const [toggleLike, { isLoading: isTogglingLike }] = useToggleAdvertisementCommentLikeMutation();
  ...
  const handleLikeComment = useCallback(async (commentId) => {
    try {
      ...
      dispatch(
        advertisementCommentsApi.util.updateQueryData(
  ...
  }, [toggleLike, dispatch, advertisementId, user.id, refetch]);
  ...
  }, [createComment, advertisementId, newComment, isCreatingComment]);
  ...
  }, [replyText, replyingTo, isCreatingComment, replyingToUser, createComment, advertisementId, findRootCommentId, comments]);
        ...
        {isReplying && (
          <View style={[styles.nestedReplyInput, { marginLeft: 8 }]}>
            <View style={styles.replyInputRow}>
              <Image
                source={{
                  uri: user?.profileImage || "https://via.placeholder.com/40x40/6C5CE7/ffffff?text=U",
                }}
  ...
  if (isLoading) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLine} />
          <Text style={styles.headerTitle}>Advertisement Comments</Text>
          <TouchableOpacity
            onPress={() => navigation.goBack()}
            style={styles.backButton}
          >
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
          ...
          <Text style={{ marginTop: 8, color: COLORS.textSecondary, textAlign: 'center', fontSize: 12 }}>
            {error?.status === 401 ? 'Please login to view comments' : 
             error?.status === 404 ? 'Advertisement not found' :
           ...
           <View style={styles.inputRow}>
             <Image
               source={{
                 uri:
                   user?.profileImage ||
                   "https://via.placeholder.com/40x40/6C5CE7/ffffff?text=U",
               }}
            ...
            <TextInput
              ...
              placeholder={replyingTo ? "Write a reply..." : "What do you think of this advertisement?"}
              placeholderTextColor={COLORS.textSecondary}
